...
export default function FourHrBiasTab({ fourHrBias, loading = false }) {
  if (loading) {
    return (
  ...
  if (!fourHrBias) return null;

  if (fourHrBias.status === 'COLLECTING') {
    return (
      <div className="bg-white dark:bg-slate-800 rounded-lg p-4 border border-neutral-200 dark:border-none">
        <div className="flex items-center gap-2 mb-3">
          <span className="text-neutral-500 text-sm font-semibold">4HR BIAS</span>
          <span className="text-xs text-neutral-400 ml-auto">Collecting...</span>
        </div>
        <div className="text-center py-4">
          <p className="text-sm text-neutral-500">{fourHrBias.message}</p>
        </div>
      </div>
    );
  }

  if (fourHrBias.error || fourHrBias.supported === false) {
    return (
      <div className="bg-white dark:bg-slate-800 rounded-lg p-4 border border-neutral-200 dark:border-none">
        <span className="text-sm text-neutral-500">{fourHrBias.error || 'Not available for this coin'}</span>
      </div>
    );
  }

  const { prediction, confidence, keyFactors, invalidation, currentPrice, generatedAt } = fourHrBias;
    ...
    <div className="bg-white dark:bg-slate-800 rounded-lg border border-neutral-200 dark:border-none p-4">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold text-neutral-900 dark:text-white">4HR BIAS</span>
          <InfoTooltip position="bottom-right">
            <div className="space-y-2 text-xs">
              <div className="font-bold text-neutral-900 dark:text-white">4-Hour Bias Prediction (BTC Only)</div>
              <div className="text-neutral-600 dark:text-slate-300">
                Flow Confluence (50%) - Price + OI + CVD alignment (1hr/2hr veto)<br/>
                OI Rate of Change (25%) - 4hr leverage dynamics<br/>
                CVD Persistence (25%) - 4hr net buying/selling pressure
              </div>
              <div className="text-neutral-500 dark:text-slate-400 pt-2 border-t border-neutral-200 dark:border-slate-600">
                Pure flow signals for scalping/day trading. Valid for 2 hours.
              </div>
            </div>
          </InfoTooltip>
        </div>
          ...
          {keyFactors?.map((factor, i) => (
            <div key={i} className="bg-neutral-50 dark:bg-slate-700/50 rounded p-2">
              <div className="flex items-center gap-1 mb-1">
                ...
                <span className="text-[10px] text-neutral-400 dark:text-slate-500 ml-auto">({factor.weight})</span>
              </div>
              <div className={`text-xs font-mono ${getFactorColor(factor.direction)}`}>
                {factor.detail}
              </div>
            </div>
          ))}
      ...
      {invalidation && invalidation.price && (
        <div className="mt-3">
          <span className={`text-xs px-2 py-1 rounded border ${invalidation.type === 'below' ? 'border-red-200 dark:border-red-800 text-red-600 dark:text-red-400' : 'border-green-200 dark:border-green-800 text-green-600 dark:text-green-400'}`}>
