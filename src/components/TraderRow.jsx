...
const TraderRow = ({ trader, rank, isSelected, onClick }) => {
    ...
    return (
        <tr className={`border-b border-neutral-200 dark:border-slate-700/50 cursor-pointer transition-colors ${isSelected ? 'bg-cyan-500/10' : 'hover:bg-neutral-50 dark:hover:bg-slate-800/50'}`} onClick={onClick}>
            <td className="py-3 px-3 text-center">
                <span className={`inline-flex items-center justify-center w-8 h-8 rounded-lg font-bold text-sm ${rank === 1 ? 'bg-yellow-500/20 text-yellow-600 dark:text-yellow-400' :
                    rank === 2 ? 'bg-slate-400/20 text-slate-500 dark:text-slate-300' :
                        rank === 3 ? 'bg-amber-600/20 text-amber-600 dark:text-amber-500' :
                            'bg-neutral-100 dark:bg-slate-700/50 text-neutral-500 dark:text-slate-400'
                    }`}>{rank}</span>
            </td>
            ...
            <td className="py-3 px-3 text-right font-mono text-neutral-900 dark:text-white text-sm">{formatUSD(trader.accountValue)}</td>
            ...
            <td className="py-3 px-3 text-center text-neutral-500 dark:text-slate-300 text-sm">{trader.positionCount || '-'}</td>
        </tr>
    );
};
